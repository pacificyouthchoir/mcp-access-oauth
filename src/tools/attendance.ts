...
export function registerAttendance(server: McpServer, getEnv: () => RockEnv) {
  server.tool(
    ...
    "Show a PYC person's recent attendance (present/absent) with dates and what they were attending — rehearsals, retreats, gala, dress rehearsals, or any other check-in. Read-only.",
    ...
    async ({ query, limit }) => {
      try {
        ...
        if (people.length > 1) return { content: [{ text: `Multiple match "${query}" — you can pass the Id:\n${people.map((p) => `- ${(p.nickName || p.firstName || "")} ${p.lastName || ""} (Id ${p.id})`).join("\n")}`, type: "text" }] };
        ...
        const rows = await rockSearch(env, "attendances", {
          where: orIds("PersonAliasId", aliasIds),
        ...
        // occurrence -> group, then resolve ANY group's real name
        const occGroup: Record<number, number> = {};
        const groupLabel: Record<number, string> = {};
        try {
          const occIds = [...new Set(rows.map((r) => r.occurrenceId).filter(Boolean))];
          if (occIds.length) {
            for (const o of await rockSearch(env, "attendanceoccurrences", { where: orIds("Id", occIds), select: "new { Id, GroupId }", limit: 50 }))
              occGroup[o.id] = o.groupId;
            const gids = [...new Set(Object.values(occGroup).filter(Boolean))];
            for (let i = 0; i < gids.length; i += 50)
              for (const g of await rockSearch(env, "groups", { where: orIds("Id", gids.slice(i, i + 50)), select: "new { Id, Name }", limit: 50 }))
        ...
        const lines = rows.map((r) => {
          ...
          const gid = occGroup[r.occurrenceId];
          const label = ENSEMBLE_BY_GROUP_ID[gid] || groupLabel[gid] || "";
          return `- ${date} — ${status}${label ? ` — ${label}` : ""}`;
        });
